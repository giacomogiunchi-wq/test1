...
std::vector<std::shared_ptr<const PartDefinition>>
DefinitionRegistry::partDefinitions() const {
  std::vector<std::shared_ptr<const PartDefinition>> result;
  result.reserve(parts_.size());
  for (const auto &[id, value] : parts_) {
    (void)id;
    result.push_back(value);
  }
  return result;
}
std::vector<std::shared_ptr<const AssemblyDefinition>>
DefinitionRegistry::assemblyDefinitions() const {
  std::vector<std::shared_ptr<const AssemblyDefinition>> result;
  result.reserve(assemblies_.size());
  for (const auto &[id, value] : assemblies_) {
    (void)id;
    result.push_back(value);
  }
  return result;
}
std::vector<std::shared_ptr<const PartRevision>>
DefinitionRegistry::partRevisions() const {
  std::vector<std::shared_ptr<const PartRevision>> result;
  result.reserve(partRevisions_.size());
  for (const auto &[id, value] : partRevisions_) {
    (void)id;
    result.push_back(value);
  }
  return result;
}
std::vector<std::shared_ptr<const AssemblyRevision>>
DefinitionRegistry::assemblyRevisions() const {
  std::vector<std::shared_ptr<const AssemblyRevision>> result;
  result.reserve(assemblyRevisions_.size());
  for (const auto &[id, value] : assemblyRevisions_) {
    (void)id;
    result.push_back(value);
  }
  return result;
...
std::vector<AssemblyOccurrence> AssemblyRuntimeGraph::occurrences() const {
  std::vector<AssemblyOccurrence> result;
